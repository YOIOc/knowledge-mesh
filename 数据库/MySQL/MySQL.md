...
问题1：为什么需要Redo Log Buffer？
	合并写入，减少磁盘IO：如果每次修改都直接写磁盘，那磁盘的随机I/O次数会非常多。Buffer的存在可以将多次小规模修改合并为一次较大规模的顺序写入。

问题2：为什么必须先写入Redo Log File到磁盘，而不是直接把Buffer Pool中的数据页写回磁盘
- 性能：顺序写 远快于 随机写
...
- ReadView（解决事务查询选择版本的问题）
	...
	2. 版本链数据访问规则（trx_id代表当前事务id）
	 ...
	 - 结合Undo Log版本链中记录的事务id，从上到下逐行带入到ReadView的数据访问规则，依次匹配，若该版本可以访问，则直接返回该版本数据；否则，则说明该版本不可见，继续顺着版本链向旧版本回溯
	- 不同隔离级别下，ReadView的生成规则是不一样，最终的访问结果也不一样
		- 读已提交：每一次执行快照读（事务中的select语句）时都会生成一份ReadView
		- 可重复读：仅在事务中第一次执行快照读时生成ReadView，后续复用
## 5.主从同步
MySQL主从复制的核心是二进制日志文件Bin Log（记录DDL-数据定义语言、DML-数据操纵语言）
- 主库在事务提交时，会把数据变更记录在二进制日志文件Bin Log中
- 从库读取主库二进制日志文件 BinLog，写入到从库的中继日志 Relay Log
- 从库重做中继日志中的事件，将变更记录到自己的数据中
## 6.分库分表
##### 6.1 什么情况下分库分表
单表数据达到1000万或超过20G
##### 6.2具体拆分策略
- 垂直分库分表
- 水平分表（Sharding-Sphere、MyCat）